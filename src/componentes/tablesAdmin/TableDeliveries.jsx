...
export const TableDeliveries = ({ deliveries, setDeliveries, getDeliveries }) => {
    ...
    const findSale = async (_id) => {
        setIsLoading(true)
        const response = await axios.get(`https://cocomadbackend.onrender.com/api/deliveries/${_id}`);
    ...
    const deleteDelivery = async (_id) => {
        ...
        await axios.delete(`https://cocomadbackend.onrender.com/api/deliveries/${_id}`, { headers });
        await getDeliveries();
        setIsLoading(false);
    };
